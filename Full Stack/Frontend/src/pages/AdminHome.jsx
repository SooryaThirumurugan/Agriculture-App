...
const AdminHome = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:8081/loans', loanDetails);
  ...
  return (
    <div>
      <AdminNavBar />
      <div className='admin-home-new-loan-container'>
        <h2>Add New Agriculture Loan</h2>
        <form onSubmit={handleSubmit} className='admin-home-new-loan-form'>
          <div>
            <label>Title:</label>
      ...
      <div className='admin-home-loan-list'>
        <h2>My Loans</h2>
        <ul>
          {loans.map((loan) => (
            <li key={loan.id}>
